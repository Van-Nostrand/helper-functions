 ...
 */
const recursiveStateCopy = <T>(oldState: T): T => {
  ...
  if (oldState instanceof Map) {
    ...
    newState = Object.keys(oldState).reduce((acc: Record<string, unknown>, cur: keyof typeof acc) => {
...
const thisIsAPlainObject = (thing: unknown): thing is Record<string | number, unknown> => {
  return Object.getPrototypeOf(thing).constructor.name === 'Object'
}

const thisIsPrimitive = (thing: unknown): thing is string | number | boolean => {
